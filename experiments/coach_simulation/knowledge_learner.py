...
## 跟 experiments/learner_simulation.cpp 的差別

那支（C++）把學習者寫成「有 P 的機率找到解」，
並用 hint_boost[3] 表示各層提示的加成。

問題是 hint_boost[POINT] = 0.55 這一行**已經假設了「指方向」有用**，
只是不知道有多少用。要驗證「這一層到底傳遞了什麼」，機率模型測不出來。
