...
/* =========================================================================
   ...
   難度標記（1–3）只用於模擬實驗，實際遊戲中不顯示。
   標的原則：
     1  規則的直接應用，看得懂就會
     2  要多想一步，或有一個常見的誤解要避開
     3  需要理解設計意圖，或答案違反直覺
   ========================================================================= */

std::vector<McqQuestion> RecapBank::forLevel(int level) {
    ...
    if (level == 1) {   // ── 接龍頭尾 ──
        ...
        qs.push_back({
            ...
            {{"一定要留著，Joker 很珍貴", false},
             {"還沒破冰的話就先破冰——不能出牌等於零分", true},
             {"一定先用掉，免得被對手搶", false},
             {"隨便，沒差", false}},
            "想想不破冰的話，你這一整局能做什麼。",
            "沒破冰就完全不能出牌，手牌只會越積越多。能破冰時先破冰，"
            "Joker 的價值要在能出牌之後才談得上。",
            3
        });
        qs.push_back({
